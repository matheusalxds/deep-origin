...
export class ExceptionFilterError implements ExceptionFilter {
  constructor(@InjectPinoLogger(ExceptionFilterError.name) private readonly logger: PinoLogger) {}
    ...
    const {
      ...
      internal = 'ISE-01',
      stackError,
    } = getErrorMessage(exception) as { source: string; internal: string; stackError: string };
    const translatedErrorMsg: string = i18n.t(t(source), { defaultValue: i18n.t(t(defaultValueMsg)) });
